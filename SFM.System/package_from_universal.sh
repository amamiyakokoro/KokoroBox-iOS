...
UNIVERSAL_APP="build/SFM.System-universal/KokoroBoxM.app"
...
build_pkg() {
	...
	rm -rf "$pkgroot" "build/component-$arch.pkg" "build/KokoroBoxM-$label.pkg"
	mkdir -p "$pkgroot"
	ditto "$app_dir" "$pkgroot/KokoroBoxM.app"
	...
	productbuild --distribution "SFM.System/distribution-$arch.xml" \
		...
		"build/KokoroBoxM-$label.pkg"
	pkgutil --check-signature "build/KokoroBoxM-$label.pkg" > /dev/null
...
for arch in arm64 x86_64; do
	...
	ditto --arch "$arch" "$UNIVERSAL_APP" "build/SFM.System-$arch-thin/KokoroBoxM.app"
	resign "build/SFM.System-$arch-thin/KokoroBoxM.app"
done

build_pkg arm64 Apple "build/SFM.System-arm64-thin/KokoroBoxM.app"
build_pkg x86_64 Intel "build/SFM.System-x86_64-thin/KokoroBoxM.app"
